			...
			<c:if test="${hasCoordinatorRole eq 'true' && hasAdminRole eq 'false'}">
				<c:if test="${assessment.status == 3 && assessment.stage == 2}">
					<div class="topBarGrey">
						<p class="AssessorP">User: <span>${assessment.benefitsDTO.userFullName}</span></p>
						<p class="siteNameP">Role: <span>${assessment.benefitsDTO.roleName}</span></p>
						<p class="versionP">Version: <span>${assessment.benefitsDTO.version}</span></p>
						<%
					...
					<div class="infoSection">
						<h2 class="siteAssessmentSubHeadings">Understanding Benefits</h2>

						<div id="coacDiv" class="infoSectionContent">
							<jsp:include page="/assessment_jsp/innerJsp/step7_understand_benefit.jsp" />
						</div>
					</div>
					...
					<div class="infoSection">
						...
						<div id="coppDiv" class="infoSectionContent">
							<jsp:include page="/assessment_jsp/innerJsp/step7_benefit_summary.jsp"/>
						</div>
					</div>
				</c:if>
			</c:if>
