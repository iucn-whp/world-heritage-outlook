		...
		<div class="topBarContent">
	       ...
	       System.out.print("EX: "+ ex.getMessage());
			...
			<div class="searchBarLabel">
				...
				<select id="assessmentCycleID" name="assessmentCycleID">
					<option value="all">All Cycles</option>
					<%--<option value="2016">2016 Cycles</option>--%>
					<option value="2017">2017 Cycles</option>
					<option value="2014">2014 Cycles</option>
				</select>
			</div>

			<div class="searchBarbutton" style="float: right">
				<input type="button" value="Search" onClick="javascript:assessmentSearch();" />
				<input name="Reset" type="reset"  value="Reset" />
				<input type="button" value="Advanced Search"  onClick="javascript:advanceSearchPopup('Assessment Query Builder');"/>
			</div>
		</div>
	</div>
</aui:form>
...
<script>
	function setAssessmentCycle(){
		var assessmentCycleIDVal = '<%= ParamUtil.getString(request, "assessmentCycleID") %>';
		if(assessmentCycleIDVal){
			document.getElementById("assessmentCycleID").value =assessmentCycleIDVal;
		}
	}
	setAssessmentCycle();
	...
	AUI().use('aui-io-request', function(A) {
	   ...
	   A.io.request('<%=assessmentSearchURL %>', {
	      ...
	      data:{
	    	 ACTION_CMD:'search',
			 assessmentCycleID: document.getElementById("assessmentCycleID").value,
