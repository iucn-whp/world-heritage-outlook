...
public class AssessmentWorkflowAction {
	 ...
	 * @param response
	 * @throws Exception
	 *             functon for assigning assessor
	 */
...
*/
 ...
 List<assessment_lang_version>  lstassessment_lang_version=assessment_lang_versionLocalServiceUtil.findByAssessmentId(assessment_id);
	...
	public static void fillFromPrevAssessmentAction(ActionRequest request,
		...
		long passessmentId = ParamUtil.getLong(request, "passessmentId");
		//long userId = ParamUtil.getLong(request, "userId");
		...
		if (assessmentId > 0) {
			...
			// sending stage id for new created version
			long newAssessment_version_id = createNewAssessmentVersion(
					assessmentId, pversionId, userId, currAssessmentCode,
					0);
	...
	public static long createNewAssessmentVersion(long assessmentId,
			long assessment_version_id, long userId, double assessmentVerCode,
			long parent_assessment_versionid) throws Exception {
		...
		if (objSite_assessment != null) {
			try {
				...
				site_assessment_versions newSite_assessment_versions = new site_assessment_versionsClp();// site_assessment_versionsLocalServiceUtil.
				...
				if (newAssessmetVersionId > 0) {
					...
					if (listprotection_management != null) {
						for (protection_management objprotection_management : listprotection_management) {
							protection_management newprotection_management = objprotection_management;
