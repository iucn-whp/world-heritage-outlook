...
import com.iucn.whp.assesment.utils.export.ExportToWord;
...
public class assementAdminPortlet extends MVCPortlet {
     ...
     * @throws Exception
     */
    public void compilationActiveconver(ActionRequest request, ActionResponse response) throws Exception {
        try {
            ...
            String projectDurationFrom = ParamUtil.getString(request, "projectDurationFrom");
            String projectDurationTo = ParamUtil.getString(request, "projectDurationTo");
            ...
            if (active_conservation != null) {
                ...
                active_conservation.setProjectDurationFrom(projectDurationFrom);
                active_conservation.setProjectDurationTo(projectDurationTo);
    ...
    public void compilationofpotentialpro(ActionRequest request, ActionResponse response) throws Exception {
        String organisationname = ParamUtil.getString(request, "organisation");
        String briefdescription = ParamUtil.getString(request, "description");
        ...
        if (potential_project != null) {
            ...
            potential_project.setProjectDurationFrom(projectDurationFrom);
            potential_project.setProjectDurationTo(projectDurationTo);
...
//		long commwider = ParamUtil.getLong(request, "commwider");
        ...
        long habitatChangeLevel = ParamUtil.getLong(request, "habitatChangeLevel");
        long habitatChangeTrend = ParamUtil.getLong(request, "habitatChangeTrend");
        long pollutionLevel = ParamUtil.getLong(request, "pollutionLevel");
        long pollutionTrend = ParamUtil.getLong(request, "pollutionTrend");
        long overExploitationLevel = ParamUtil.getLong(request, "overExploitationLevel");
        long overExploitationTrend = ParamUtil.getLong(request, "overExploitationTrend");
        long climateChangeLevel = ParamUtil.getLong(request, "climateChangeLevel");
        long climateChangeTrend = ParamUtil.getLong(request, "climateChangeTrend");
        long invasiveSpeciesLevel = ParamUtil.getLong(request, "invasiveSpeciesLevel");
        long invasiveSpeciesTrend = ParamUtil.getLong(request, "invasiveSpeciesTrend");
...
//    		benefitsobj.setWider_community(commwider);
            ...
            benefitsobj.setHabitatChangeLevel(habitatChangeLevel);
            benefitsobj.setHabitatChangeTrend(habitatChangeTrend);
            benefitsobj.setPollutionLevel(pollutionLevel);
            benefitsobj.setPollutionTrend(pollutionTrend);
            benefitsobj.setOverExploitationLevel(overExploitationLevel);
            benefitsobj.setOverExploitationTrend(overExploitationTrend);
            benefitsobj.setClimateChangeLevel(climateChangeLevel);
            benefitsobj.setClimateChangeTrend(climateChangeTrend);
            benefitsobj.setInvasiveSpeciesLevel(invasiveSpeciesLevel);
            benefitsobj.setInvasiveSpeciesTrend(invasiveSpeciesTrend);

            benefitsobj.setComment(comment);
    ...
    public void serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
        ...
        } else if (cmd != null && cmd.equalsIgnoreCase("DOWNLOAD_WORD_DOCUMENT")) {
            try {
                downloadWordDocument(resourceRequest, resourceResponse);
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        } else if (cmd != null && cmd.equals("checkFillFields")) {
            checkFillFields(resourceRequest, resourceResponse);
        } else {
    ...
    private String getPreviousVersion(String assessment_versionId) {
        ...
        try {
            ...
            previousVersionCode = Double.valueOf(previousVersionCode);
...
//			if(lstCThreat.size()==0 || lstPThreat.size()==0 || lstSumCur.get(0).getThreat_rating()==0 ||lstSumPot.get(0).getThreat_rating()==0 || lstSumOver.get(0).getThreat_rating()==0)
            ...
            //lstpromgmtchecklkp.size()-1 - because we have 16 elements but we ignored one because so the customer
            // wants(it's topic_id = 15(this id is static and the same in all DB))).
            /*
            >if (lstProMgmt.size()!=lstpromgmtchecklkp.size()) // || lstprotmgmtbestpract==null || lstprotmgmtbestpract.isEmpty()
				bitCode = bitCode+"1";
			else
				bitCode = bitCode+"0";
            *
            * */
            ...
            for (int i = 0; i < lstProMgmt.size(); i++) {
                if (lstProMgmt.get(i).getTopic_id() == 15) {
                    lstProMgmt.remove(lstProMgmt.get(i));
                }
            }

            for (int i = 0; i < lstpromgmtchecklkp.size(); i++) {
                if (lstpromgmtchecklkp.get(i).getTopic_id() == 15) {
                    lstpromgmtchecklkp.remove(i);
                }
            }

            boolean check = true;
            protection_management enf = null;

            for (protection_management management : lstProMgmt) {

                if (management.getTopic_id() == 16) {
                    check = false;
                    enf = management;
                }
            }

            if (lstProMgmt.size() != lstpromgmtchecklkp.size()) {

                if (lstProMgmt.size() + 1 == lstpromgmtchecklkp.size() && check) {
                    bitCode = bitCode + "0";
                } else {
                    bitCode = bitCode + "1";
                }
                ...
                try {
                    if (enf != null && !enf.getJustification().isEmpty()) {
                        bitCode = bitCode + "0";
                    } else {
                        bitCode = bitCode + "1";
                    }
...
//                bitCode = bitCode+"0";
            ...
            /*if (lstProMgmt.size()!=lstpromgmtchecklkp.size()){
                ...
                if (check){
                    ...
                    if (lstProMgmt.size() + 1 == lstpromgmtchecklkp.size() && )
                    bitCode = bitCode+"0";
                }
            }else{
                bitCode = bitCode+"0";
...
*/
            /*if (lstProMgmt.size() != lstpromgmtchecklkp.size())
                if (lstProMgmt.size() == 15 && lstpromgmtchecklkp.size() == 16) {
                    bitCode = bitCode + "0";
                } else {
                    bitCode = bitCode + "1";
                }// || lstprotmgmtbestpract==null || lstprotmgmtbestpract.isEmpty()*/
...
//			String strBitCode = getBitCode(bitCode);
    ...
    public void deleteCurrentRecordFromTable(ActionRequest request, ActionResponse response)
        ...
        try {
            ...
            if (tableName.contains("Delete conservation project")) {
            ...
            if (tableName.contains("Delete site need")) {
    ...
    public void updatestep2PotentialThreat(ActionRequest request, ActionResponse response)
        ...
        try {
            ...
            String threadExtentValue = insideSite ? ParamUtil.getString(request, "threatExtent") : "";
            ...
            if (objassessing_threats_potential != null) {
                ...
                objassessing_threats_potential.setThreadExtentValue(threadExtentValue);
    ...
    public void updatestep2CurrentThreat(ActionRequest request, ActionResponse response)
        ...
        try {
            ...
            if (objassessing_threats_current != null) {
                ...
                objassessing_threats_current.setThreadExtentValue(threadExtentValue);
    ...
    public void approveAndPublishAssessmentAction(ActionRequest actionRequest, ActionResponse actionResponse)
            throws IOException {
        try {
            AssessmentWorkflowAction.approveAssessmentAction(actionRequest, actionResponse);
            AssessmentWorkflowAction.publishAssessmentAction(actionRequest, actionResponse);
            AssessmentWorkflowAction.approveAssessmentAction(actionRequest, actionResponse);
            AssessmentWorkflowAction.publishAssessmentAction(actionRequest, actionResponse);
        } catch (Exception ex) {
            ex.printStackTrace();
            SessionErrors.add(actionRequest, "error-assessment");
            actionResponse.sendRedirect("/web/iucn/assessment");
        }
    }
    ...
    private void downloadWordDocument(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
            throws PortalException, SystemException, IOException {
        ExportToWord.exportToWordDocument(resourceRequest, resourceResponse);
    }

    public void newInformationAction(ActionRequest request,
                                     ActionResponse response) throws Exception {
        String siteId = request.getParameter("siteId");

        whp_sites whp_sites = whp_sitesLocalServiceUtil.getwhp_sites(Long.parseLong(siteId));
        whp_sites.setInformation_updating(true);
        whp_sitesLocalServiceUtil.updatewhp_sites(whp_sites);
    }


    public void checkFillFields(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws IOException {
        ...
        try {
            String selectedPkey = resourceRequest.getParameter("keyVal");
            tempassessmentVersionId = resourceRequest.getParameter("versionIdVal");
            ...
            resourceResponse.getWriter().write("false");
        }
        resourceResponse.setContentType("application/json");
        resourceResponse.getWriter().write(String.valueOf(isFilled));
    }
}