...
public class AssessmentActionUtil {
        ...
        try {
            ...
            if (site_assessment_versionsList != null && site_assessment_versionsList.size() > 0) {
                ...
                for (int i = 0; i < site_assessment_versionsList.size(); i++) {
                    ...
                    if (!site_assessment_versionstemp.getIs_translated()) {
    ...
    public static boolean hasLinkVisible(long assessment_id, long currentUserId, String actionLink) {
		...
		/*Changes make by NIRAV*/

        Boolean isCooardinator = false;
        List<Role> roleList = null;
        try {
            roleList = RoleLocalServiceUtil.getUserRoles(currentUserId);
            for (Role role : roleList) {
                if (role.getName().equalsIgnoreCase(AssessmentContstant.COORDINATOR_ROLE)) {
                    isCooardinator = true;
                }
            }
		...
		/*Changes make by NIRAV*/
        else if (AssessmentContstant.ACTION_EDIT_PUBLISHED  //Edit while Coordinator login
                .equalsIgnoreCase(actionLink)) {
            if (isCooardinator) {
                if (AssessmentContstant.STAGE_ASSSEMENT == currentStageId
        ...
        //permission to delete an assessment added by kamal on 8 Aug 2013
            ...
            isVisible = true;
        } else if (AssessmentContstant.ACTION_PDF.equalsIgnoreCase(actionLink)) {
            isVisible = true;
...
//				}
            for (Role role : objUser.getRoles()) {
            ...
            if (roleList != null && roleList.size() > 0) {
					 ...
					 */
                if (roleList.contains(AssessmentContstant.ADMIN_ROLE) || roleList.contains(AssessmentContstant.WHP_STAFF_ROLE) || roleList.contains(AssessmentContstant.COORDINATOR_ROLE) || roleList.contains(AssessmentContstant.READ_ONLY_ASSESS)) {
                    ...
                    site_assessmenList = site_assessmentLocalServiceUtil.getAllActiveSiteAssessment();
						 ...
						 * START: Change added by Jayesh for role implementation
						 */
                    if (roleList.contains(AssessmentContstant.REVIEWER_ROLE)) {
                        HashSet<site_assessment> site_assessment_Hashset = new HashSet<site_assessment>();

//							   site_assessment_Hashset.addAll(site_assessmentLocalServiceUtil.getActiveSiteAssessmentForReviewer(userid));
                        site_assessment_Hashset.addAll(site_assessmentLocalServiceUtil.getActiveAssessmentByUserId(userid));
                        site_assessmenList = new ArrayList<site_assessment>();
                        for (site_assessment assessment : site_assessment_Hashset) {
                            site_assessmenList.add(assessment);
                        }
                    } else {
