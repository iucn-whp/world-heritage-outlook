...
<c:if test="<%= AssessmentActionUtil.hasButtonVisible(assessmentId,user.getUserId(),AssessmentContstant.ACTION_SUBMIT_ASSESSMENT ) %>">
    <input type="button" value="<%=AssessmentContstant.ACTION_SUBMIT_ASSESSMENT %>"
           onClick="javascript:validatePopup('submitForReviewAction','Are you sure to submit the Assessment Questionnaire for review?','Review');"/>

</c:if>

<c:if test="<%= AssessmentActionUtil.hasButtonVisible(assessmentId,user.getUserId(),AssessmentContstant.ACTION_SUBMIT_REVIEW) %>">
    <input type="button" value="<%=AssessmentContstant.ACTION_SUBMIT_REVIEW %>"
           onClick="javascript:validatePopup('reviewCompletedAction','Are you sure to submit the Assessment Questionnaire for approve?','Approve');"/>
</c:if>

<c:if test="<%= AssessmentActionUtil.hasButtonVisible(assessmentId,user.getUserId(),AssessmentContstant.ACTION_APPROVE) %>">
    <input type="button" value="<%=AssessmentContstant.ACTION_APPROVE %>"
           onClick="javascript:validatePopup('approveAssessmentAction','Are you sure to submit the Assessment Questionnaire for publish?','Publish');"/>
</c:if>
...
<c:if test="<%= AssessmentActionUtil.hasButtonVisible(assessmentId,user.getUserId(),AssessmentContstant.ACTION_SUBMIT_TRANSLATION) %>">
    <input type="button" value="<%=AssessmentContstant.ACTION_SUBMIT_TRANSLATION %>"
           onClick="javascript:validatePopup('translationCompleateAction','Are you sure to submit the Assessment Questionnaire?');"/>
</c:if>
