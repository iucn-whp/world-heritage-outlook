...
<%
    ...
    List<current_threat_values> testCurrVal = new ArrayList<current_threat_values>();
    List<current_threat_values> testBioVal = new ArrayList<current_threat_values>();
    ...
    try {
        ...
        if (selectedPkey != null && !selectedPkey.isEmpty()) {
            ...
            threadExtentValue = curretn_threat.getThreadExtentValue();
            ...
            for (current_threat_values wh : lstCurrentThreatValTemp) {
                if (!wh.getIs_biodiv_whval()) {
                    ...
                    testBioVal.add(wh);
                    lstCurrentThreatBioVal.add(wh.getWhvalues_id());
                }
            }
            ...
            curretn_threat.setThreadExtentValue("");
    ...
    >
        ...
        <table class="formBlock">
            <tr>
                ...
                <td>
                    <select name="threatCategoryTemp" id="threatCategoryTemp"
                        ...
                        <option value="<%=lstThreatCategory.get(i).getThreatCategoryid() %>"
                                title="<%=lstThreatCategory.get(i).getThreat_category() %>">
                        ...
                        <%
                            if (lstCurrentAssessmentCat != null && !lstCurrentAssessmentCat.isEmpty()) {
                                for (current_threat_assessment_cat current_threat : lstCurrentAssessmentCat) {
                                    ...
                                    String threatSubCatStr = "";
                                    if (current_threat.getSub_cat_id() != 7){
                                        threatSubCatStr = threat_subcategories_lkpLocalServiceUtil.getthreat_subcategories_lkp(current_threat.getSub_cat_id()).getThreat_sub_category();
                                    }else{
                                        continue;
                                    }
            ...
            <tr>
                ...
                <td class="valueTD">
                    <div class="floatLeftX">
                        ...
                        <aui:input type="checkbox" onClick="insideSiteShowBlock()" cssClass="insideSite"
                                   name="insideSite" id="insideSite" label=""
                                   value="<%=curretn_threat.getInside_site() %>"/>
                    </div>
                    <div class="floatLeftX">
                        <label class="popuplbl">Outside Site:&nbsp;&nbsp;</label>
                        <aui:input cssClass="outsideSite" type="checkbox" name="outsideSite" id="outsideSite" label=""
            ...
            <tr id="insideSiteBlock">
                ...
                <td>
                    <div>
                        Extent of threat inside site:
                    </div>

                    <div style="width: 33%; float: left">
                        <label><input type="checkbox" id="throughout"
                                      <c:if test="<%=\"Throughout(>50%)\".equals(threadExtentValue)%>">checked</c:if>
                                      onClick="selectOnlyThis(this)" data-thread-extent="throughout"/> Throughout(>50%)</label>
                    </div>
                    <div style="width: 33%; float: left">
                        <label><input type="checkbox" id="widespread"
                                      <c:if test="<%= \"Widespread(15-50%)\".equals(threadExtentValue)%>">checked</c:if>
                                      onClick="selectOnlyThis(this)" data-thread-extent="widespread"/>
                            Widespread(15-50%)</label>
                    </div>

                    <div style="width: 33%; float: left">
                        <label><input type="checkbox" id="scattered"
                                      <c:if test="<%= \"Scattered(5-15%)\".equals(threadExtentValue)%>">checked</c:if>
                                      onClick="selectOnlyThis(this)" data-thread-extent="scattered"/>
                            Scattered(5-15%)</label>
                    </div>
                    <div style="width: 33%; float: left">
                        <label><input type="checkbox" id="localised"
                                      <c:if test="<%= \"Localised(<5%)\".equals(threadExtentValue)%>">checked</c:if>
                                      onClick="selectOnlyThis(this)" data-thread-extent="localised"/>
                            Localised(<5%)</label>
                    </div>
                    <div style="width: 33%; float: left">
                        <label><input type="checkbox" id="notKnownThreat"
                                      <c:if test="<%= \"Extent of threat not known\".equals(threadExtentValue)%>">checked</c:if>
                                      onClick="selectOnlyThis(this)" data-thread-extent="notKnownThreat"/> Extent of
                            threat not known</label>
                    </div>
                    <div style="width: 33%; float: left">
                        <label><input type="checkbox" id="notApplicable"
                                      <c:if test="<%= \"Not applicable\".equals(threadExtentValue)%>">checked</c:if>
            ...
            <tr>
                ...
                <td class="valueTD" style="padding:1px 0px 0px 0px;">
                    <div class="checkboxListforThreat" id="valueAffected">
                        ...
                        <input type="checkbox" name="valueAffectedwh" id="valueAffectedwh" checked
                               value="<%=value_id %>"> <%= lstWhValues.get(i).getValues()%><br>
                        <!-- </option> -->
                        <%} else { %>

                            <%-- <option value = "<%=wh_id %>"> --%>
                        <input type="checkbox" name="valueAffectedwh" id="valueAffectedwh"
                               value="<%=value_id %>"> <%= lstWhValues.get(i).getValues()%><br>
                        <!-- </option> -->
                        <%
                                }
            ...
            <tr>
                ...
                <td class="valueTD" style="padding:1px 0px 0px 0px;">
                    <div class="checkboxListforThreat" id="bioValues">
                        ...
                        %>
                            <%-- <option value = "<%=wh_id %>" title="<%= lstWhValues.get(i).getValues()%>"> --%>
                        <input type="checkbox" name="valueAffectedbio" id="valueAffectedbio" checked
                               value="<%=value_id %>"> <%= lstBioValues.get(i).getValue()%><br>
                        <!-- </option> -->
                        <%} else { %>

                            <%-- <option value = "<%=wh_id %>"> --%>
                        <input type="checkbox" name="valueAffectedbio" id="valueAffectedbio"
                               value="<%=value_id %>"> <%= lstBioValues.get(i).getValue()%><br>
                        <!-- </option> -->
                        <%
                                }
    ...
    jQuery(function () {
        jQuery("#threatCategoryTemp").change(function () {
            ...
            for(int i = 0; i < lstThreatSubCategoryTemp.size(); i++){
                if (lstThreatSubCategoryTemp.get(i).getId() == 7){
                    lstThreatSubCategoryTemp.remove(i);
                }
            }
...
<script>
    ...
    function validatestep2CurForm() {
        ...
        var assessment = $('#<portlet:namespace/>currentAssessment').val(),
                valueAffectedwh = [],
                insideSiteCheck = $(".insideSite .aui-field-input-choice").is(':checked'),
                outsideSiteCheck = $(".outsideSite .aui-field-input-choice").is(':checked'),
                insideSiteBlockInputs = $("#insideSiteBlock :input");
        ...
        } else if (!insideSiteCheck) {
            if (!outsideSiteCheck) {
                isValid = false;
                alert("Please indicate whether threat is inside or outside the site, or both.");
            } else if (valueAffectedwh == "" && valueAffectedbio =="") {
                isValid = false;
                alert("Please enter details on values affected");
        ...
        } else if (!$(insideSiteBlockInputs).is(':checked')) {
            isValid = false;
            alert("Please indicate extent of threat inside site.");
        } else if (valueAffectedwh == "" && valueAffectedbio =="") {
            isValid = false;
            alert("Please enter details on values affected");
        } else if (justification == "") {
