...
<table class="tab_data" cellpadding="0" cellspacing="0">
    ...
    <tr>
        ...
        <td id="<%=lstthreats_current.get(0).getThreat_sum_cur_id() %>_SC_JF_TD"><%=lstthreats_current.get(0).getOverall_current() %>
        </td>

        <td id="<%=lstthreats_current.get(0).getThreat_sum_cur_id() %>_SC_RT_TD"><span
                class="<%=threatRating.getCssClass() %>"><%=assessment %></span></td>
        <%} %>
        <td class="editIcons hideAdminAction">
            <div class="editDelete">
                <a href="#"
                   onclick="javascript:dynamicPopup('${UpdateUrl}','<%=currentThreat %>','Overall Assessment of current Threats',event); return false;"
                   class="edit">edit</a>
            </div>
        </td>
    </tr>
    ...
    <tr>
        ...
        <%
            ...
            String assessment = threat_rating_lkpLocalServiceUtil.getthreat_rating_lkp(rating).getRating();
        %>
        <td id="<%=lstthreats_potential.get(0).getThreat_sum_potential_id() %>_SP_JF_TD"><%=lstthreats_potential.get(0).getOverall_potential() %>
        </td>
        <td id="<%=lstthreats_potential.get(0).getThreat_sum_potential_id() %>_SP_RT_TD"><span
                class="<%=threatRating.getCssClass() %>"><%=assessment %></span></td>
        <%} %>
        <td class="editIcons hideAdminAction">
            <div class="editDelete">
                <a href="#"
                   onclick="javascript:dynamicPopup('${UpdateUrl}','<%=potentialThreat %>','Overall Assessment of Potential Threats',event); return false;"
                   class="edit">edit</a>
            </div>
        </td>
    </tr>
    ...
    <tr>
        ...
        <%
            ...
            String assessment = threat_rating_lkpLocalServiceUtil.getthreat_rating_lkp(rating).getRating();
        %>
        <td id="<%=lstthreats_overall.get(0).getThreat_sum_overall_id() %>_SO_JF_TD"><%=lstthreats_overall.get(0).getOverall_threat() %>
        </td>
        <td id="<%=lstthreats_overall.get(0).getThreat_sum_overall_id() %>_SO_RT_TD"><span
                class="<%=threatRating.getCssClass() %>"><%=assessment %></span></td>
        <%} %>
        <td class="editIcons hideAdminAction">
            <div class="editDelete">
                <a href="#"
                   onclick="javascript:dynamicPopup('${UpdateUrl}','<%=overallThreat %>','Overall Assessment of Threats',event); return false;"
                   class="edit">edit</a>
            </div>
        </td>
    </tr>
