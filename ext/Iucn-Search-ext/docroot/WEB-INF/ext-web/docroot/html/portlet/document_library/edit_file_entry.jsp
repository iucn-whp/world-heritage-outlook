...
<table cellspacing="0" cellpadding="0" class="fileEntryTable">
<td valign="top" class="fileEntryTableTD">
		<aui:input name="file" class="inputField01" type="file" >
			<aui:validator name="acceptFiles">
				'<%= StringUtil.merge(PrefsPropsUtil.getStringArray(PropsKeys.DL_FILE_EXTENSIONS, StringPool.COMMA)) %>'
			</aui:validator>
		</aui:input>
		
		<aui:select changesContext="<%= true %>" label="Site name" id="sitename" name="sitename"  class="inputField01">
	...
	<%
		...
		<aui:input name="title" class="inputField01">
			...
			<c:if test="<%= (folder == null) || (folder.getModel() instanceof DLFolder) %>">
				...
				<c:choose>
					<c:when test="<%= !cmd.equals(Constants.ADD) && (dlFileEntryTypes.size() < 1) %>">
						<aui:select class="inputField01" changesContext="<%= true %>" label="document-type" name="fileEntryTypeId" onChange='<%= renderResponse.getNamespace() + "changeFileEntryType();" %>'>
		...
		<td valign="top" class="fileEntryTableTDRight">
...
<c:if test="<%= fileEntry == null %>">
		...
		<aui:button-row>
			...
			String publishButtonLabel = "Upload"; //"publish";
...
<aui:script>
 ...
 <!-- var file=document.getElementById("_20_file").files.length; -->

var file=document.getElementById("_20_file").value.length;
 ...
 if(document.getElementById("_20_title").value=="") 
	{
	...
	tbtitle = document.getElementById("_20_title").value.length;
	...
	//added for com
	//var fullcom=document.getElementById("scch_Full_COM").value;
	var fullcom=document.getElementById("dyyd_Full_COM").value;
	        var pattern = (/^[0-9]+[Cc]+[oO]+[mM]+[A-Za-z0-9-'.,&@:?!()$#/\\]*$/) ;

if(fullcom!="")
{
        
if(!pattern.test(fullcom)) {
            alert(" Full COM must be  alphanumeric value (like 1212COM)");
            return false;
...
<script type="text/javascript">
	$(document).ready(function(){
		$('#_20_description').height(50);
		$('#_20_description').width(300);
