...
<div class="lfr-header-row" id="test_header">
					...
					<c:when test="<%= ((folderId == rootFolderId) && !expandFolder) || ((folder != null) && (folder.isRoot() && !folder.isDefaultRepository() && !expandFolder)) %>">
						...
						<li style="display: <%= (request.isUserInRole("Reviewer")) ? "none": "" %>" class="folder <%= (navigation.equals("home") && (folderId == rootFolderId) && (fileEntryTypeId == -1)) ? "selected" : StringPool.BLANK %>">
							...
							<c:if test="<%= (foldersCount > 0) %>">
								<a class="expand-folder" data-expand-folder="<%= Boolean.TRUE.toString() %>" data-folder-id="<%= rootFolderId %>" data-view-entries="<%= Boolean.FALSE.toString() %>" href="<%= viewDocumentsHomeURL.toString() %>">
									<liferay-ui:icon cssClass="expand-folder-arrow" image="expand" message="expand" />
								</a>
							</c:if>

						<a class="browse-folder" data-expand-folder="<%= Boolean.TRUE.toString() %>" data-folder-id="<%= rootFolderId %>" data-view-entries="<%= Boolean.FALSE.toString() %>" href="<%= viewDocumentsHomeURL.toString() %>">
						
								<liferay-ui:icon image="../aui/home" message="" />
								<!--Added by PN  -->
								<script>$('#test_header').hide();</script>
								<!--Added to hide header row  -->
						...
						<c:if test="<%= rootFolderId == DLFolderConstants.DEFAULT_PARENT_FOLDER_ID %>">
							...
							<c:if test="<%= themeDisplay.isSignedIn() %>">
								...
								<div style="display:none">
									<a class="browse-folder"  data-navigation="mine" data-expand-folder="<%= Boolean.TRUE.toString() %>" data-view-folders="<%= Boolean.FALSE.toString() %>" href="<%= viewMyDocumentsURL.toString() %>">
					...
					<c:otherwise>
						...
						<li class="folder">
							<a class="expand-folder" data-direction-right="<%= Boolean.TRUE.toString() %>" data-folder-id="<%= String.valueOf(parentFolderId) %>" data-view-entries="<%= Boolean.FALSE.toString() %>" href="<%= viewURL.toString() %>">
								<liferay-ui:icon cssClass="expand-folder-arrow" image="collapse" message="collapse" />
							</a>
							...
							<a class="browse-folder" data-direction-right="<%= Boolean.TRUE.toString() %>" data-folder-id="<%= String.valueOf(parentFolderId) %>" href="<%= viewURL.toString() %>">
								<liferay-ui:icon message="up" src='<%= themeDisplay.getPathThemeImages() + "/arrows/01_up.png" %>' />
									<!--Added By PN  -->
									<script>$('#test_header').show();</script>
									<!--Added for showing header ROW  -->
						...
						%>
							...
							<li class="folder <%= (curFolder.getFolderId() == folderId) ? "selected" : StringPool.BLANK %>">
								...
								<!--Added By PN for subfolder view on folder click-->
								<a class="browse-folder" data-expand-folder="<%= Boolean.TRUE.toString() %>" data-folder="<%= Boolean.TRUE.toString() %>" data-folder-id="<%= String.valueOf(curFolder.getFolderId()) %>" data-title="<%= curFolder.getName() %>"  href="<%= viewURL.toString() %>">
									<c:choose>
								...
								<!--Added By PN-->
								<c:if test="<%= foldersCount == 0 %>">
								<a class="browse-folder" data-view-folders="<%= Boolean.FALSE.toString() %>" data-folder="<%= Boolean.TRUE.toString() %>" data-folder-id="<%= String.valueOf(curFolder.getFolderId()) %>" data-title="<%= curFolder.getName() %>"  href="<%= viewURL.toString() %>">
									<c:choose>
										<c:when test="<%= (foldersCount + fileEntriesCount) > 0 %>">
											<liferay-ui:icon image="folder_full_document" />
										</c:when>
