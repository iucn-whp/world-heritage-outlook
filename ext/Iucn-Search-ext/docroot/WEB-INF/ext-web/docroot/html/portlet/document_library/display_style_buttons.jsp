...
<c:if test="<%= displayViews.length > 1 %>">
	<aui:script use="aui-dialog,aui-dialog-iframe">
		...
		function onButtonClick(displayStyle) {
			...
			config['<portlet:namespace />docType'] = '<%= HtmlUtil.escapeJS(docType) %>';
			config['<portlet:namespace />docCategory'] = '<%= HtmlUtil.escapeJS(docCategory) %>';
			config['<portlet:namespace />docSite'] = '<%= HtmlUtil.escapeJS(docSite) %>';
			config['<portlet:namespace />docAuthor'] = '<%= HtmlUtil.escapeJS(docAuthor) %>';
			config['<portlet:namespace />docpubDate'] = '<%= HtmlUtil.escapeJS(docpubDatestr) %>';
			config['<portlet:namespace />docUploadDate'] = '<%= HtmlUtil.escapeJS(docUploadDatestr) %>';
			config['<portlet:namespace />docFull_COM'] = '<%= HtmlUtil.escapeJS(docFull_COM) %>';
			config['<portlet:namespace />docCOM'] = '<%= HtmlUtil.escapeJS(docCOM) %>';
